...
class RemotionLocalRenderProvider(BaseRenderProvider):
    ...
    def renderMedia(self, job: dict[str, Any], payload: dict[str, Any]) -> dict[str, Any]:
        ...
        # Resolve other template properties passed from front-end
        for key in [
            "subtitle", "logoUrl", "logo_url", "watermarkText", "watermark_text",
            "themeVariant", "theme_variant", "transcriptLines", "transcript_lines",
            "images", "videoClips", "video_clips", "musicUrl", "music_url",
            "musicVolume", "music_volume", "musicStartTime", "music_start_time",
            "musicEndTime", "music_end_time", "sfxLayers", "sfx_layers"
        ]:
            if key in payload:
                normalized_key = {
                    "logo_url": "logoUrl",
                    ...
                    "sfx_layers": "sfxLayers"
                }.get(key, key)
                props[normalized_key] = payload[key]
...
class VoiceboxTTSProvider(BaseAudioRenderProvider):
    """
    Voicebox local AI Voice Generation (Text-to-Speech) provider.
    Integrates with Jamie Pine's Voicebox.sh local REST API.
    """
    provider_id = "voicebox_tts"
    canonical_id = "voicebox"
    ...
    def renderAudio(self, job: dict[str, Any], payload: dict[str, Any]) -> dict[str, Any]:
        ...
        voice = clean_text(payload.get("voice")) or "default"
        title = clean_text(payload.get("title")) or clean_text(job.get("title")) or "Voicebox Render"

        base_url = clean_text(os.getenv("VOICEBOX_API_URL")) or "http://localhost:8000"
        url = f"{base_url}/api/tts"
        
        request_payload = {
            "text": text,
            "voice": voice,
            "engine": clean_text(payload.get("engine")) or "kokoro"
        }
        ...
        try:
            ...
            logger.warning("Local Voicebox API connection failed: %s. Using stub generated output.", e)
            audio_bytes = b"STUB_VOICEBOX_AUDIO_BYTES_MARKER"
        except Exception as e:
        ...
        return {
            ...
            "message": "Audio render completed via local Voicebox TTS.",
        }
...
class MediaEngine:
    def __init__(self, store: MediaStateStore | None = None) -> None:
        ...
        self.audio_render_providers: dict[str, BaseAudioRenderProvider] = {
            ...
            VoiceboxTTSProvider.provider_id: VoiceboxTTSProvider(),
            VoiceboxTTSProvider.canonical_id: VoiceboxTTSProvider(),
        }
