...
const ensureCommsTables = () => {
  ...
  db.mailboxes = db.mailboxes || [
    { id: 'mailbox-primary', tenant_id: DEFAULT_TENANT_ID, workspace_id: DEFAULT_WORKSPACE_ID, name: 'Relationship HQ', address: 'mission@aiocrm.local', provider: 'mock-email', created_at: isoMinutesAgo(10080), updated_at: isoMinutesAgo(90) },
    { id: 'mailbox-growth', tenant_id: DEFAULT_TENANT_ID, workspace_id: DEFAULT_WORKSPACE_ID, name: 'Growth Desk', address: 'growth@aiocrm.local', provider: 'mock-email', created_at: isoMinutesAgo(8640), updated_at: isoMinutesAgo(180) }
  ];
  db.channels = db.channels || [
    { id: 'channel-email', tenant_id: DEFAULT_TENANT_ID, workspace_id: DEFAULT_WORKSPACE_ID, type: 'email', label: 'Email', provider: 'mock-email', created_at: isoMinutesAgo(10080), updated_at: isoMinutesAgo(90) },
  ...
  const seeds = [
    {
      id: 'thread-jenna-launch', channel: 'email', mailbox: 'mailbox-primary', contact: contacts[0], companyId: contacts[0]?.company_id || companies[0]?.id || null,
      subject: 'Launch sequencing and executive narrative', title: 'Jenna wants a tighter launch story.', status: 'waiting_on_us', flags: { high_intent: true, hot_lead: true, needs_human: true },
      priority: 'critical', score: 96, owner: 'ECHO', assignee: 'STRIKER', automation: 'manual', nextFollow: isoMinutesAgo(-180),
      summary: 'Jenna is close to approving the next phase but wants a sharper launch plan.', next: 'Send a milestone-based rollout and leadership summary.',
      unresolved: ['Confirm launch date', 'Confirm approvers'], implications: ['Enterprise upsell potential'],
      messages: [
        { id: 'msg-jenna-1', direction: 'inbound', sender: 'Jenna Best', email: contacts[0]?.email, body: 'We are close. I need a tighter rollout plan and a clearer story for leadership before I approve the next phase.', created: isoMinutesAgo(180), status: 'received' },
        { id: 'msg-jenna-2', direction: 'outbound', sender: 'ECHO', email: 'mission@aiocrm.local', body: 'Understood. I am preparing a milestone-led path and a cleaner executive narrative for review.', created: isoMinutesAgo(90), status: 'sent' }
      ]
    },
    {
      id: 'thread-sarah-demo', channel: 'email', mailbox: 'mailbox-growth', contact: contacts[3], companyId: contacts[3]?.company_id || companies[2]?.id || null,
      subject: 'Enterprise demo follow-up', title: 'Sarah is aligned on value but waiting on procurement.', status: 'waiting_on_them', flags: { high_intent: true, hot_lead: true, follow_up_due: true },
      priority: 'high', score: 84, owner: 'STRIKER', assignee: 'STRIKER', automation: 'automated', nextFollow: isoMinutesAgo(-240),
      summary: 'The demo landed. Procurement timing is the only blocker.', next: 'Send a concise procurement-forward follow-up with booking option.',
      unresolved: ['Security review owner'], implications: ['Possible flagship finance account'],
      messages: [
        { id: 'msg-sarah-1', direction: 'outbound', sender: 'STRIKER', email: 'growth@aiocrm.local', body: 'Appreciate your time today. Attached is the enterprise overview and deployment path.', created: isoMinutesAgo(900), status: 'sent' },
        { id: 'msg-sarah-2', direction: 'inbound', sender: 'Sarah Chen', email: contacts[3]?.email, body: 'This looks solid. I need to line up procurement and security review timing.', created: isoMinutesAgo(360), status: 'received' }
      ]
    },
    {
      id: 'thread-michael-sms', channel: 'sms', mailbox: 'mailbox-primary', contact: contacts[4], companyId: contacts[4]?.company_id || companies[3]?.id || null,
      subject: 'Healthcare compliance follow-up', title: 'Michael needs a fast compliance answer.', status: 'new', flags: { needs_human: true, at_risk: true },
      priority: 'high', score: 91, owner: 'ALPHA', assignee: 'ECHO', automation: 'manual', nextFollow: isoMinutesAgo(-60),
      summary: 'Compliance concern could stall the deal without a fast reply.', next: 'Respond with a compliance-forward answer and offer a short call.',
      unresolved: ['Audit trail proof'], implications: ['Could reopen a qualified deal'],
      messages: [
        { id: 'msg-michael-1', direction: 'inbound', sender: 'Michael Rodriguez', email: contacts[4]?.email, body: 'Need to know if your workflow logs are detailed enough for compliance review this week.', created: isoMinutesAgo(18), status: 'received' }
      ]
    },
    {
      id: 'thread-emily-internal', channel: 'internal', mailbox: 'mailbox-primary', contact: contacts[5], companyId: contacts[5]?.company_id || companies[4]?.id || null,
      subject: 'Trial expansion plan', title: 'Internal planning around Emily’s conversion path.', status: 'scheduled', flags: { follow_up_due: true },
      priority: 'medium', score: 68, owner: 'ALPHA', assignee: 'ECHO', automation: 'automated', nextFollow: isoMinutesAgo(-120),
      summary: 'The trial is healthy but the buying trigger is still vague.', next: 'Prepare a tailored follow-up tied to active usage.',
      unresolved: ['Decision timeline'], implications: ['Could become an education playbook'],
      messages: [
        { id: 'msg-emily-1', direction: 'system', sender: 'ALPHA', email: 'system@aiocrm.local', body: 'Create a follow-up pack for EduLearn focused on active feature adoption and a 30-day conversion path.', created: isoMinutesAgo(600), status: 'logged' }
      ]
    }
  ];
  seeds.forEach((seed) => {
...
export const createThread = ({ subject, channelType = 'email', contactId = null, companyId = null, body = '', status = 'new', assignee = 'ECHO' }) => {
  ...
  db.threads.push({ id: threadId, tenant_id: DEFAULT_TENANT_ID, workspace_id: DEFAULT_WORKSPACE_ID, mailbox_id: 'mailbox-primary', channel_type: channelType, source_provider: channelType === 'email' ? 'mock-email' : channelType === 'sms' ? 'manual-sms' : 'system', subject: subject || `${channelType.toUpperCase()} thread`, generated_title: subject || `${contact ? `${contact.first_name} ${contact.last_name}` : 'New'} conversation`, status, ai_flags: normalizeFlags({ needs_human: true }), ai_priority: 'medium', priority_score: 70, owner: 'ECHO', assignee, contact_id: contact?.id || null, company_id: resolvedCompanyId, automation_state: 'manual', last_activity_at: now, next_follow_up_at: null, created_at: now, updated_at: now });
  ...
  if (body) sendThreadMessage({ threadId, body, channelType, senderName: DEFAULT_USER, senderEmail: 'mission@aiocrm.local' });
...
export const sendThreadMessage = ({ threadId, body, channelType, senderName = DEFAULT_USER, senderEmail = 'mission@aiocrm.local', recipients = [], direction = 'outbound' }) => {
  const thread = findThread(threadId);
  if (!thread) throw new Error('Thread not found');
...
export const simulateInbound = ({ channelType = 'email', contactId = null } = {}) => {
  ...
  return sendThreadMessage({ threadId: target.id, body, channelType, senderName: contact ? `${contact.first_name} ${contact.last_name}`.trim() : 'Incoming Contact', senderEmail: contact?.email || 'contact@inbox.local', recipients: ['mission@aiocrm.local'], direction: 'inbound' });
};
...
export const ensureFormSubmissionThread = ({ form, contactId, submissionId, formData }) => {
  ...
  updateBriefRecord(thread.id, { summary: `${form.name} created a new inbound signal that should be triaged in Comms.`, disposition: 'New lead or contact signal', recommended_next_step: 'Review the submission, enrich CRM context, and draft a first response.', unresolved_questions: ['Confirm best reply channel', 'Decide whether to trigger a workflow'], crm_implications: [`Submission linked to ${form.name}`], reasoning_cues: ['Thread originated from form ingestion'] });
