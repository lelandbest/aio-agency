...
import { History, Loader2, RotateCw, Search, Workflow } from 'lucide-react';
    ...
    <div className="max-h-[20rem] overflow-y-auto p-3 crm-scroll-hidden">
      ...
      <div className="space-y-2">
        {runs.map((run) => {
          ...
          const isActive = activeRunId && activeRunId === run.id;
          return (
            <div
              key={run.id}
              className={`rounded-xl border p-3 ${isActive ? 'border-violet-500/40 bg-violet-500/10' : 'border-[var(--color-border)]/60 bg-[var(--color-bg-primary)]/70'}`}
            >
